...
private struct IsThis{
  func aPrime(_ integer : UInt64) -> Bool{
    ...
    default:
      for i in 2...UInt64(sqrt(Double(integer))){
        if integer % i == 0{
          return false
        }
      }
...
class PrimeEngine{
  ...
  func checkPrime(_ integer : UInt64) -> Bool {
    return IsThis().aPrime(integer)
  }
  func cancelCurrentOperation() {
    self.cancel = true
  }
  
  func isThereAnyOperationProgress() -> Bool{
  ...
  func abc(){
    ...
    let def = SortThePrimes(withRepetition: 100, startingFrom: 600851475144, toUpwards: false)
    var index = 0
    for i in def {
      if i != 0 {
      print("\(index) : \(i)")
